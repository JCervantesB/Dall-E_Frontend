...
const CrearPost = () => {
  ...
  const generarImagen = async () => {
    if(formulario.instrucciones) {
      try {
        ...
        const respuesta = await fetch('https://dall-e-clon.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
      ...
      Swal.fire({
        icon: 'error',
        text: 'Por favor completa el campo de instrucciones'
      })
    }
  };
  ...
  const handleSubmit = async (e) => { 
    ...
    if(formulario.nombre && formulario.instrucciones && formulario.imagen) {
      try {
        setCargando(true);
        const respuesta = await fetch('https://dall-e-clon.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
